...
describe('Author Management Tests', () => {
  ...
  it('should visit the authors, create a new author, upload an image, and verify data through GraphQL', () => {
    cy.visit('https://redsys-stage.sandsmedia.com/authors');

    cy.contains('Create new author').should('be.visible').click();
    cy.wait(5000);

    cy.get('#frc-name--1782415253').should('be.visible').click().type(formData.title);
    cy.get('#frc-slug--1317495733').should('be.visible').click().type(formData.slug);
    cy.get('#frc-forename-283834671').should('be.visible').click().type(formData.forename);
    cy.get('#frc-surname-1717441631').should('be.visible').click().type(formData.surname);
