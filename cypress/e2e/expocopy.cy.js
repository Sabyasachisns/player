...
describe('Create Expo and Check Publish Status', () => {
 ...
 const { expo, partnerType, pageUrls, eventSelector } = this.testData;
        ...
        // cy.get('.list').should('be.visible').contains(eventSelector).click();
        // cy.wait(5000);
    ...
    //     cy.get(`td[data-value="${expo.endDateValue}"]`).eq(3).click();
    //     cy.get('button[type="Submit"]').scrollIntoView().should('be.visible').click();
    //    // cy.get('.titleStringValue').should('contain', 'Internal Name:');

    //     // Click on the first <a> link under an <h4> tag where href starts with "/expos/"
    ...
    //     cy.get('div.titleStringValue').eq(2).should('contain', 'Published');
    ...
    //     cy.get('[name="internalIdentifier"]').click().type(`unique${uniqueId0}`);
    ...
    //     cy.get('[name="internalIdentifier"]').click().type(`unique${uniqueId1}`);
    ...
    //     // cy.visit("https://redsys-stage.sandsmedia.com/expos/920d8e0bd38acb813bb90a6f?option=partnerTypes#");
    //     // cy.get(".dropzone-base").attachFile(filePath);
    ...
    //     //cy.get('svg[viewBox="0 0 24 24"]').eq(5).click();
    ...
    //     // const filePath1='samplefile.jpg';
    //     // cy.visit("https://redsys-stage.sandsmedia.com/expos/920d8e0bd38acb813bb90a6f?option=partnerTypes#");
    //     // cy.get('.dropzone-base input[type="file"]').selectFile('C:\Users\skanar\Downloads.samplefile.jpg');
    //      cy.get('.i18n-message ').contains('Submit').click();
    ...
    //     // cy.get('#515908bcc0f631504bdfff08').click();
    //     // cy.go(-3);
    ...
    //     cy.get('#frc-forename-283834671').should('be.visible').click().type(`unique${uniqueId}`);
    //     cy.get('#frc-surname-1717441631').should('be.visible').click().type(`uniqu${uniqueId}`);
    //     cy.get('#frc-company-178924925').type(`company${uniqueId}`);
    //     cy.get('#frc-position-1324593687').type(`Manager`);
    //     cy.get('#frc-email-197005799').type(`email${uniqueId}@hotmail.com`);
    ...
    //     //Upload a file for staff

    //     // cy.get('.detail').eq(2).within(() => {
    //     //     // Find the a[href="#"] inside the third .detail element and click it
    //     //     cy.get('a[href="#"]').click();
    //     //   });
    ...
    //     cy.get('input[type="file"]').eq(1).attachFile('samplefile.jpg');
    //     cy.wait(3000)
    ...
    //     cy.get('a[href="#"]').eq(2).click();
    //    // cy.get('#9cc77e6982c3ec9994f172fc').click();
    //     //cy.get('a[href="#"]').eq(6).click();
    //     // cy.get('a[href="#"]').eq(9).click();
    ...
    cy.get('h4 a[href^="/expos/"]').first().click();
         ...
         cy.wait(3000);
         cy.get('a[href="#"]').eq(6).click();
         cy.get('a[href="#"]').eq(7).click(); ////////////////////////////////// make it 7 

        cy.wait(23000);
        const fileName3 = 'img.jpg';  
        cy.get('input[type="file"]').eq(1).attachFile('img.jpg');
        const fileName4 = 'img.jpg';  
        cy.get('input[type="file"]').eq(1).attachFile('img4.jpg');
        cy.wait(3000);
        cy.get('.css-1hwfws3').eq(1).select(0);
        cy.get('.i18n-message ').contains('Submit').click();
        cy.get('button').contains('Publish').click({force: true});
        cy.reload();
